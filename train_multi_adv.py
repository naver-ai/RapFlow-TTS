...
from model.discriminator import Discriminator2d
...
def main(args, cfg):
    ...
    discriminator  = Discriminator2d(**cfg.model.gan)
    discriminator  = nn.SyncBatchNorm.convert_sync_batchnorm(discriminator)  ## convert to sync batchnorm
    discriminator  = DDP(discriminator.to(device), device_ids=[rank])
    optimizer_disc = torch.optim.Adam(discriminator.parameters(), lr=float(cfg.train.lr))
    scaler_disc    = GradScaler(enabled=cfg.train.amp)
    ...
    assert cfg.train.stage ==3, f'Training stage for adversarial learning should be 3'
    ...
    if cfg.train.stage == 3:
        ...
        logger.info(f'---load previous weigths {ckpt_name} and new optimizer for training stage 3---')
    ...
    if cfg.resume is not None:
        ...
        discriminator.module.load_state_dict(checkpoint['state_dict_disc'])
    ...
    for epoch in range(epoch_start, cfg.train.epoch+1):
        ...
        for i, batch in enumerate(tqdm(train_loader)):
            ...
            with autocast(enabled=cfg.train.amp):
                dur_loss, prior_loss, diff_loss, x1, x1_pred = model(x, x_lengths, y, y_lengths, spks=spk, out_size=cfg.train.out_size)
                if cfg.train.encoder_freeze:
                    dur_loss, prior_loss = torch.tensor(0).to(diff_loss.device), torch.tensor(0).to(diff_loss.device) 

                ############ discriminator loss ############
                x1.requires_grad_()
                out_real, _ = discriminator(x1.unsqueeze(1))
                loss_real   = adv_loss(out_real, 1, use_mse=cfg.train.use_mse)
                out_fake, _ = discriminator(x1_pred.detach().unsqueeze(1))
                loss_fake   = adv_loss(out_fake, 0, use_mse=cfg.train.use_mse)
                if cfg.train.use_reg:
                    loss_reg = r1_reg(out_real, x1)
                else:
                    loss_reg = torch.tensor(0).to(diff_loss.device)
                d_loss = loss_real + loss_fake + loss_reg * cfg.train.lambda_reg        

                optimizer_disc.zero_grad()
                scaler_disc.scale(d_loss).backward()
                scaler_disc.unscale_(optimizer_disc)
                torch.nn.utils.clip_grad_norm_(discriminator.parameters(), max_norm=cfg.train.max_grad)
                scaler_disc.step(optimizer_disc)
                scaler_disc.update()
                #########################################
                                        
                ############ generator loss ############
                out_fake, f_fake = discriminator(x1_pred.unsqueeze(1))
                loss_adv         = adv_loss(out_fake, 1, use_mse=cfg.train.use_mse)
                
                # feature matching loss
                if cfg.train.use_fm:
                    with torch.no_grad():
                        _, f_real   = discriminator(x1.unsqueeze(1))
                    loss_fm = feature_loss(f_real, f_fake)
                else:
                    loss_fm = torch.tensor(0).to(diff_loss.device) 
                        
                loss_adv  = cfg.train.lambda_adv * loss_adv + cfg.train.lambda_fm * loss_fm
                diff_loss = cfg.train.lambda_diff * diff_loss
                loss      = sum([dur_loss, prior_loss, diff_loss, loss_adv])
        ...
        total_adv_loss   = total_adv_loss / (i+1)
        total_disc_loss  = total_disc_loss / (i+1)
        train_loss       = (total_dur_loss + total_diff_loss + total_prior_loss) / 3
        
        if rank==0:
            logger.info(f"(Step: {cur_step:07d}) | train Loss: {train_loss:.4f} | dur Loss: {total_dur_loss:.4f} | diff Loss: {total_diff_loss:.4f} | prior Loss: {total_prior_loss:.4f} | disc Loss: {total_disc_loss:.4f} | adv Loss: {total_adv_loss:.4f}")
            if cfg.logging:
                ...
                run['train/disc loss'].append(total_disc_loss)
                run['train/adv loss'].append(total_adv_loss)
                
            validate(model, discriminator, val_loader, run, logger, cur_step, cfg, cfg.device)
            ...
            if epoch % cfg.train.syn_every == 0:    
                synthesize(model.module, discriminator.module, vocoder, denoiser, val_dataset, logger, cfg, device, sample_size=10)
        dist.barrier()
...
def validate(model, discriminator, data_loader, run, logger, cur_step, cfg, device):
...
def synthesize(model, discriminator, vocoder, denoiser, dataset, logger, cfg, device, sample_size=10):
