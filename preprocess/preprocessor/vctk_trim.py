...
def prepare_align(config):
    ...
    wav_path  = config["path"]["wav_path"] 
    text_path = wav_path.replace('wav48_silence_trimmed', 'txt')
    out_path  = config["path"]["raw_path"]
    sampling_rate = config["preprocessing"]["audio"]["sampling_rate"]
    
    total_meta_list = [] # wav_path, out_wav_path, text, speaker, base_name
    for speaker in tqdm(os.listdir(wav_path)):
        ...
        for file_name in os.listdir(os.path.join(wav_path, speaker)):
            ...
            tmp_text_path = os.path.join(text_path, speaker, "{}.txt".format(base_name[:-5]))  # remove _mic*
            tmp_wav_path  = os.path.join(wav_path, speaker, "{}.flac".format(base_name)) 
            tmp_out_path  = os.path.join(out_path, speaker, "{}.wav".format(base_name))            
            

            if 'mic1' in file_name and os.path.isfile(tmp_text_path) and os.path.isfile(tmp_wav_path):
